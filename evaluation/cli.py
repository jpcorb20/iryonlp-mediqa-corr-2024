...
# This is a refactoring (from Jean-Philippe Corbeil <jcorbeil@microsoft.com>) to have a CLI for evaluation.
...
class NLGMetrics(object):
    ...
    def compute(self, references, predictions, counters):
        ...
        if 'BERTSCORE' in self.metrics:
            bs_path = os.path.join(self.base_model_path, "deberta-large-mnli")
            bertscore_params = dict(model_type=bs_path, lang='en', device ='cpu' , verbose=True, rescale_with_baseline=True, num_layers=24)
            _, _, bertScore_F1 = bertscore(predictions, references, **bertscore_params)
        ...
        if 'BLEURT' in self.metrics:
            bleurt_path = os.path.join(self.base_model_path, "BLEURT-20")
...
def filer_ref(ref: dict, keys: set) -> dict:
    return {k: v for k, v in ref.items() if k in keys}


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("-f", "--submission_file", type=str, required=True, help="Submission file path")
    parser.add_argument("-c", "--reference_csv_file", type=str, required=True, help="Reference CSV file path.")
    parser.add_argument("-m", "--model_dir",  type=str, required=True)
    parser.add_argument("-fr", "--filter_ref_from_submission",  action="store_true")
    args = parser.parse_args()

    submission_file = args.submission_file
    reference_csv_file = args.reference_csv_file
    ...
    if args.filter_ref_from_submission:
        reference_keys = set(reference_corrections.keys())
        candidate_keys = set(candidate_corrections.keys())
        key_diff = list(reference_keys.difference(candidate_keys))
        print("Missing keys: %s" % ",".join(key_diff))
        reference_corrections = filer_ref(reference_corrections, candidate_keys)
        reference_flags = filer_ref(reference_flags, candidate_keys)
        reference_sent_id = filer_ref(reference_sent_id, candidate_keys)
    ...
    metrics = NLGMetrics(model_dir=args.model_dir)
